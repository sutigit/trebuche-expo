...
export default function PromptTextInput({ text, setText, onSubmit, disabled, alert }: { text: string, setText: (text: string) => void, onSubmit: () => void, disabled: boolean, alert: boolean }) {
            ...
            <FormControl className='w-full' isInvalid={alert} size="md" isDisabled={disabled} isReadOnly={disabled}>
                ...
                <FormControlError>
                    ...
                    <FormControlErrorText>
                        Herra, ei voi olla tyhjää
                    </FormControlErrorText>
                </FormControlError>
            </FormControl>
            <Button className='mt-4 bg-indigo-300' onPress={onSubmit} disabled={disabled}>
